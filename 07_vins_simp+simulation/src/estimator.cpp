...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
    ...
    //首先判断像素点的视差，运动了多少个单位，如果超过了阈值，则判断为新的关键帧，把最早的关键帧丢掉。
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))
        marginalization_flag = MARGIN_OLD;
    else //不是关键帧，直接把新帧丢弃掉，但是保留IMU信息
    ...
    if (ESTIMATE_EXTRINSIC == 2) //如果IMU和相机外参不知道，没有提供，需要标定旋转。
    {
        ...
        if (frame_count != 0)
        {
            ...
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric))
            {
                ...
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
                ESTIMATE_EXTRINSIC = 1; //把状态标为1,说明对vins系统可以初始化了。
            }
        }
    }
    ...
    if (solver_flag == INITIAL)
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if (result)  //如果完成了，就是true，变成了一个非线性优化问题。
            {
...
void Estimator::solveOdometry()
{
    ...
    {
        TicToc t_tri;
        f_manager.triangulate(Ps, tic, ric);  //把特征点给三角化
...
//ceres需要把vector变成double指针的操作，而我们其实不需要这个操作，我们操作还是对vector进行操作的
//为了节省工作量，这部分内容不进行改动。
{
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
...
void Estimator::MargNewFrame() //把视觉特征丢掉，只留imu信息。
{
...
{
    backend::LossFunction *lossfunction;
    lossfunction = new backend::CauchyLoss(1.0); //对 H矩阵求解过程中加一个权重。
    ...
    //  设置好顶点以后，要设置边的约束，包括视觉约束、IMU约束和先验约束。
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        shared_ptr<backend::VertexPose> vertexCam(new backend::VertexPose());
        Eigen::VectorXd pose(7);
    ...
    problem.Solve(10);
    //  求解后可能又会有一些先验的操作，这里需要把先验的变量保存下来。
...
void Estimator::backendOptimization()
{
    ...
    problemSolve();
    // 优化后的变量处理下自由度
    double2vector(); //把第一帧的变化旋转回去，算出增量，再把所有的变量变回去。因为求解时没有固定第一维，也没有加先验。
