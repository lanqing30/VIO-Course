...
void System::PubImageData(double dStampSec, string &sim_img,int i)
{
    if (!init_feature)
    {
    ...
    //trackerData[0].readImage(img, dStampSec); //把第一幅图像中提取特征点，保存到cur_pts之类的变量里去。
    //需要看一下readimage中怎么读取的图片和检测的特征
    ...
    //{
       // bool completed = false;
        ...
        //if (!completed)
            //break;
    //}
    if (PUB_THIS_FRAME)//每来一张图像，获取的特征保存到feature_points中去
    {
        pub_count++;
        shared_ptr<IMG_MSG> feature_points(new IMG_MSG());
        feature_points->header = dStampSec;
        vector<set<int>> hash_ids(NUM_OF_CAM);
        ifstream every_point;
	    every_point.open(sim_img.c_str());
        if (!every_point.is_open())
	    {
		    cerr << "Failed to open image file! " << sim_img << endl;
		    return;
	    }
        cout<<"input success!"<<i<< endl;
        string scam_lines;
        double x_w,y_w,z_w,m_w;
        double curptx,curpty;
        int i=0;
        while (std::getline(every_point, scam_lines) && !scam_lines.empty())
        {
            std::istringstream ssImuData(scam_lines);
            ssImuData >> x_w >> y_w >> z_w >> m_w >>curptx >> curpty;
            feature_points->points.push_back(Vector3d(curptx, curpty, 1));
        ...
        //{
            //auto &un_pts = trackerData[i].cur_un_pts; //归一化坐标通过cur_un_pts保存
            ...
            //{
                ...
                //{
                    ...
                    //double y = un_pts[j].y;
                    //double z = 1;
                    //featurepoints中保存归一化平面上的坐标，还存特征的id，像素坐标，速度
        ...
        //}
            ...
            {
                m_buf.lock();
                feature_buf.push(feature_points);//两个buff，一个保存imu的，一个是这里保存特征的
...
{
    //这个函数就是在根据时间戳卡数据，卡出图像和对应的imu数据。之后获取了数据，会去ProcessBackEnd函数中进一步处理。
...
{
    shared_ptr<IMU_MSG> imu_msg(new IMU_MSG()); //把imu的数据读进来，放到一个自定义的类里
    ...
    //放到一个队列里，imu的数据是高频的，相比图像是比较多的，每个图像对应很多IMU数据，根据图像之间时间戳卡出imu数据形成约束
    // cout << "1 PubImuData t: " << fixed << imu_msg->header 
    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++) 
            {
                ...
                int feature_id = v / NUM_OF_CAM;  //numofcam是相机个数1，单目系统是1
            ...
            estimator.processImage(image, img_msg->header); //系统状态估计器将处理图像数据。
            ...
            if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
            {
                ...
                ofs_pose << fixed << dStamp << " " << p_wi(0)<< " "<< p_wi(1) << " " << p_wi(2)<< " " << q_wi.x()<< " " << q_wi.y()<< " " << q_wi.z()<< " " << q_wi.w()<< endl;
            }
        }
        m_estimator.unlock();
    }
