...
MotionData IMU::MotionModel(double t)
{
    ...
    double K2 = K*K;
    Eigen::Vector3d ddp( -K2 * ellipse_x * cos(K*t),  -K2 * ellipse_y * sin(K*t), -z*K1*K1*K2 * sin(K1 * K * t));     // position二阶导数,即加速度
...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 0; i < imudata.size()-1; ++i) {    
        ...
        /// imu 动力学模型 欧拉积分
        //Eigen::Vector3d dtheta_half =  imupose.imu_gyro * dt /2.0;  //1/2*w*δt
        //dq.w() = 1;
        //dq.x() = dtheta_half.x();
        //dq.y() = dtheta_half.y();
        //dq.z() = dtheta_half.z();
        ...
        /// 中值积分       
        Eigen::Vector3d dtheta_half= (imupose.imu_gyro * dt +imudata[i+1].imu_gyro*dt)/4.0; //之所以除以4.0，是因为比欧拉积分多了一个2.0
        dq.w() = 1;  //开始更新当前时刻的dq
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        Eigen::Quaterniond nextQwb=Qwb * dq; //i+1时刻Qwb
        Eigen::Vector3d acc_w=((Qwb * (imupose.imu_acc) + gw) + (nextQwb *(imudata[i+1].imu_acc)+ gw))/2.0;  //i时刻的a
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;  //i+1时刻的Pwb
        Vw = Vw + acc_w * dt; //i+1时刻的Vw
        Qwb=nextQwb;
