...
import os
filepath="/home/zlq/Desktop/VIO-Course/02_vio_data_simulation/bin_hw2"  #表示当前所处的文件夹上一级文件夹的绝对路径
...
quaterntions1 = []
timestamp1 = []
with open(filepath + '/imu_int_pose_uler.txt', 'r') as f:  # imu_pose   imu_spline
...
with open(filepath + '/imu_int_pose_mid.txt', 'r') as f:  # imu_pose   imu_spline
    ...
    for line in data:
        ...
        # timestamp12.append( numbers_float[0])
        # quaterntions12.append( [numbers_float[tx_index+6], numbers_float[tx_index+3],numbers_float[tx_index+4],numbers_float[tx_index+5]   ] )   # qw,qx,qy,qz
        position12.append([numbers_float[tx_index], numbers_float[tx_index + 1], numbers_float[tx_index + 2]])
...
xyz = zip(*position)
xyz1 = zip(*position1)
xyz12=zip(*position12)
xyz2 = zip(*position2)
print
ax.plot(xyz[0], xyz[1], xyz[2], label='gt')
ax.plot(xyz1[0], xyz1[1], xyz1[2], label='imu_int_Euler')
ax.plot(xyz12[0], xyz12[1], xyz12[2], label='imu_int_mid')
