...
bool Problem::AddVertex(std::shared_ptr<Vertex> vertex)
{
    ...
    if (problemType_ == ProblemType::SLAM_PROBLEM)
    {
        ...
        {
            ResizePoseHessiansWhenAddingPose(vertex); //添加
        }
    }
    return true;
}
...
bool Problem::AddEdge(shared_ptr<Edge> edge)
{
    ...
    // std::cout << "edge_id: " << edge->Id() << std::endl;

    for (auto &vertex : edge->Verticies())
    {
        vertexToEdge_.insert(pair<ulong, shared_ptr<Edge>>(vertex->Id(), edge));
        // std::cout << "vertexToEdge_id: " << vertex->Id() << std::endl;
    }
    return true;
}
...
void Problem::SetOrdering()
{
    ...
    for (auto vertex : verticies_)
    {
        ...
        if (IsPoseVertex(vertex.second))
        {
            std::cout << "vertex " << vertex.second->Id() << " order: " << vertex.second->OrderingId() << std::endl;
        }
    }
    ...
    if (problemType_ == ProblemType::SLAM_PROBLEM)
    {
        ...
        std::cout << "all_pose_dimensions: " << all_pose_dimension << std::endl;
        for (auto landmarkVertex : idx_landmark_vertices_)
        {
            landmarkVertex.second->SetOrderingId(
                landmarkVertex.second->OrderingId() + all_pose_dimension);
            std::cout << "landmark " << landmarkVertex.second->Id() << " order: " << landmarkVertex.second->OrderingId() << std::endl;
        }
    }
    // std::cout << "ordering_poses: " << ordering_poses_ <<std::endl;
    // std::cout << "ordering_landmarks: " << ordering_landmarks_ <<std::endl;
    // std::cout << "ordering_generic: " << ordering_generic_ <<std::endl;
...
void Problem::MakeHessian()
{
    ...
    // {
    //     std::cout << "edge_num: "<<edge.first << "  ";
    //     std::cout << "vertex a: "<<edge.second->GetVertex(0)->Id()<< "  ";
    //     std::cout << "vertex b: "<<edge.second->GetVertex(1)->Id()<< "  ";
    //     std::cout << "vertex c: "<<edge.second->GetVertex(2)->Id()<< std::endl; //打印看这个map加载的顺序真奇怪
    // }

    for (auto &edge : edges_) //每一条边都是一个残差，都会对H产生贡献
    {
        edge.second->ComputeResidual();
        edge.second->ComputeJacobians();

        auto jacobians = edge.second->Jacobians(); // std::vector  jacobians 2*1  2*6  2*6
        auto verticies = edge.second->Verticies(); // std::vector  VertexInverseDepth, VertexPose, VertexPose
        assert(jacobians.size() == verticies.size());
        for (size_t i = 0; i < verticies.size(); ++i)
        {
            auto v_i = verticies[i]; //顶点
            if (v_i->IsFixed())
                continue; // Hessian 里不需要添加它的信息，也就是它的雅克比为 0

            auto jacobian_i = jacobians[i];      //顶点对应的Jacobian
            ulong index_i = v_i->OrderingId();   //找到这个顶点在 Ｈessian矩阵中的 row 的位置
            ulong dim_i = v_i->LocalDimension(); //矩阵块　row的维度

            MatXX JtW = jacobian_i.transpose() * edge.second->Information();
            // std::cout << "jacobian_i \r\n "<< jacobian_i << std::endl;
            // std::cout << "Information \r\n "<< edge.second->Information() << std::endl;//单位矩阵２＊２
            for (size_t j = i; j < verticies.size(); ++j) //主对角线
            {
                auto v_j = verticies[j];

                if (v_j->IsFixed()) //固定了，就相当于相对应的Jacobian为０，所以不会对Ｈ矩阵产生贡献
                    continue;

                auto jacobian_j = jacobians[j];
                ulong index_j = v_j->OrderingId();   //找到这个顶点在 Ｈessian矩阵中的 col 位置
                ulong dim_j = v_j->LocalDimension(); //矩阵块 col 的维度

                assert(v_j->OrderingId() != -1);
                MatXX hessian = JtW * jacobian_j;
                // 所有的信息矩阵叠加起来
                // TODO:: home work. 完成 H index 的填写.
                // std::cout << "with "<<index_i <<", "<< index_j <<"add hessian block "<<dim_i <<"*"<<dim_j <<std::endl;
                H.block(index_i, index_j, dim_i, dim_j).noalias() += hessian; //从(index_i,index_j)位置插入　dim_i*dim_j 的矩阵
                if (j != i)
                {
                    // 对称的下三角
                    // TODO:: home work. 完成 H index 的填写.
    ...
    Hessian_ = H;
    b_ = b;
    t_hessian_cost_ += t_h.toc(); // time cost
    ...
    Hessian_.topLeftCorner(ordering_poses_, ordering_poses_) += H_prior_; //仅仅　pose对应的块
...
void Problem::SolveLinearSystem()
{
    ...
    else
    {
        ...
        H_pp_schur_ = Hessian_.block(0, 0, reserve_size, reserve_size) - tempH * Hmp; // Hpp - Hpm*Hmm^-1*Hmp
        b_pp_schur_ = bpp - tempH * bmm;                                              //bpp - Hpm*Hmm^-1 *bmm
        ...
        {
            H_pp_schur_(i, i) += currentLambda_; //加上阻尼因子
        }

        int n = H_pp_schur_.rows() * 2;                      // 迭代次数
        delta_x_pp = PCGSolver(H_pp_schur_, b_pp_schur_, n); //
        delta_x_.head(reserve_size) = delta_x_pp;
        // std::cout << "delta_x_pp: \r\n" << delta_x_pp.transpose() << std::endl;
...
void Problem::UpdateStates()
{
    for (auto vertex : verticies_)
    {
        ...
        VecX delta = delta_x_.segment(idx, dim);//切出这个状态量
        vertex.second->Plus(delta);//顶点状态量的更新
    }
    // TODO: what is that mean?
    if (err_prior_.rows() > 0)
    {
...
bool Problem::IsGoodStepInLM()
{
    ...
    double rho = (currentChi_ - tempChi) / scale;// 更新阻尼因子的rho
    if (rho > 0 && isfinite(tempChi)) // last step was good, 误差在下降
    {
        double alpha = 1. - pow((2 * rho - 1), 3);//阻尼因子的更新策略
