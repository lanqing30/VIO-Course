...
struct Frame
{
    ...
    unordered_map<int, Eigen::Vector3d> featurePerId; // 该帧观测到的特征以及特征id　(归一化相机平面坐标)
};
...
void GetSimDataInWordFrame(vector<Frame> &cameraPoses, vector<Eigen::Vector3d> &points)
{
    ...
    for (int j = 0; j < featureNums; ++j)
    {
        ...
        {
            Eigen::Vector3d Pc = cameraPoses[i].Rwc.transpose() * (Pw - cameraPoses[i].twc); //特征点在cameraPoses[i]下坐标
            Pc = Pc / Pc.z();                                                                // 归一化图像平面
...
int main()
{
    ...
    Eigen::Quaterniond qic(1, 0, 0, 0); // 假设IMU与相机外参对齐
    ...
    for (size_t i = 0; i < points.size(); ++i)
    {
        ...
        {
            Eigen::Vector3d pt_i = cameras[0].featurePerId.find(i)->second;      // 每个特征点在 source Camera 下的归一化坐标
            Eigen::Vector3d pt_j = cameras[j].featurePerId.find(i)->second;      // 每个特征点在 mearsurement Camera 下的归一化坐标
    ...
    {
        std::cout << "after opt, point " << k << " : gt " << 1. / points[k].z() << " \t with_noise "
